...
source = source.rename(
    {'id': 'reply_id', 'text': 'text_source'}, axis='columns')
source = pd.concat([source, annotation['is_rumour']], axis=1)

reaction['in_reply_to_status_id_str'] = reaction['in_reply_to_status_id_str'].fillna(
    0)
reaction = reaction.astype({'in_reply_to_status_id_str': 'int64'})
reaction = reaction.rename(
    {'in_reply_to_status_id_str': 'reply_id'}, axis='columns')

df = pd.merge(reaction, source, on='reply_id', how='left')
df = df.drop(df[df['text_source'].isna() == True].index, axis=0)
df = df.reset_index(drop=True)

df[['text', 'text_source', 'is_rumour']].to_json(
    './dataset/tweets.json', orient='records')
